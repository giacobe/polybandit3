...
create_binary_non_printable_file() {
  ...
  generate_binary_non_printable_chars() {
    # Generate a random byte value between 0 and 255 and write it to the file
    for i in $(seq 1 "$1"); do
      dd if=/dev/urandom bs=1 count=1 status=none > "$output_file"
    done
  }
  ...
  echo "Binary file $output_file created with $file_size bytes containing non-printable ASCII characters."
}
